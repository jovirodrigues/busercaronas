...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=busercaronas.example.com
...
workon busercaronas  # Change this to your project's name
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development busercaronas npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop busercaronas
    docker rm busercaronas
    docker run --name busercaronas -d --env-file /home/ubuntu/busercaronas.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/busercaronas:/dkdata \
        busercaronas start_web.sh
}
...
echo_green "Welcome to busercaronas's dev env"
